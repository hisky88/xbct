...
def doCollect(url):
  ...
  for b in bMap:#['ff']:#
    ...
    clrScrpt = "javascript:var%20bcc=document.createElement('script');"+\
        "bcc.setAttribute('src','http://localhost/xbct/xbct.js');"+\
...
def moveFiles(config):
  ...
  for f in files:
    if f[-3:] == "png":
      shutil.copy(config["image_folder"]+"/"+f, "C:/xampp/htdocs/xbct_report/screenshots/")
      shutil.copy(config["image_folder"]+"/"+f, "C:/XBCT/analysis/Screenshots/")

if __name__ == "__main__":
  if (len(sys.argv) < 2):
    print "Usage: collectData.py url"
    exit()
  url = sys.argv[1]
  doCollect(url)