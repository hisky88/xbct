...
public class VariableElementIdentifier extends CrossBrowserTest {
	...
	public VariableElementIdentifier(int[] browserTestId, int refBrowserTestId,
			HashMap[] map) throws SQLException {
		super(browserTestId, refBrowserTestId, map);
	}

	public boolean VChk(int refBrowserTestId, int browserTestId, int[] boxi,
			int[] boxj) {
		...
		String tmp = "get_emd.exe " + WDConstants.SC_DIR + refBrowserTestId
				+ ".png " + WDConstants.SC_DIR + browserTestId + ".png "
				+ boxi[0] + " " + boxi[1] + " " + boxi[2] + " " + boxi[3] + " "
				+ boxj[0] + " " + boxj[1] + " " + boxj[2] + " " + boxj[3];
	...
	public void visualAnalysis(int[] browserTestId, int refBrowserTestId) {
		for (int i = 1; i <= browserTestId.length; i++) { // Ln3
			// TODO: Skipping Line 5 and 6 needs to be done after algo is
			// complete, Levenshetin changes, cache EMD data
			...
			for (int x = nodeTraversalArr.length - 1; x > 0; x--) {
				...
				Node nodej = (Node) rmap_index[i - 1].get(new Integer(k)); // rmap
																			// index's
																			// value
																			// is
																			// the
				...
				if (node.getDomid().equals(nodej.getDomid())
						&& node.getContenthash().equals(nodej.getContenthash())
						&& node.getName().equals(nodej.getName())
						&& node.isClickable() == nodej.isClickable()
						&& node.isVisible() == nodej.isVisible()
						&& node.getXpath().equals(nodej.getXpath())
						&& node.getX0() == nodej.getX0()
						&& node.getY0() == nodej.getY0()
						&& nodej.getX1() == node.getX1()
						&& node.getY1() == node.getY1()) {
					// Do nothing
				} else {
					markAsND(refBrowserTestId, node);
				}
				if (node.getContenthash().equals("no hash")) {
					if (!VChk(refBrowserTestId, browserTestId[i - 1],
							getBoxArr(node), getBoxArr(nodej))) {
						markAsND(refBrowserTestId, node);
					}
				}
				/*
				 * if(!VChk(refBrowserTestId, browserTestId[i-1],getBoxArr(node)
				 * , getBoxArr(nodej))){ markAsND(refBrowserTestId,node); }
				 */
	...
	private void markAsND(int refB, Node node) {
		this.db.insQuery("UPDATE domdata SET non_det = 1 WHERE testid=" + refB
				+ " and id=" + node.getId());
		// Grey out refb
		int nodeX0 = Math.max(0, node.getX0());
		int nodeX1 = Math.max(0, node.getX1());
		...
		int nodeH = nodeY1 - nodeY0;
		if (nodeW > 0 && nodeH > 0) {
			String tmp = "grey_out.exe " + WDConstants.SC_DIR + refB + ".png "
					+ nodeX0 + " " + nodeY0 + " " + nodeX1 + " " + nodeY1;
			sysCall(tmp);
		}
